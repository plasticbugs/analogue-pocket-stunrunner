...
RBF = os.path.join(ROOT, "projects", "output_files", "stunrun_pocket.rbf")
PKG = os.path.join(ROOT, "pkg", "pocket")
OUT = os.path.join(ROOT, "release", "pocket")
CORE_ID = "plasticbugs.stunrun"
...
# release contains everything needed to produce stunrun.rom.
for extra in ("stunrun.mra", "README.md", os.path.join("tools", "mra_build.py")):
...
print("build the ROM with:  python3 mra_build.py stunrun.mra stunrun.zip")

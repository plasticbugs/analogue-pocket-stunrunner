...
| Synthesis (Quartus 18.1, 5CEBA4) | `./build-local.sh` | — | **fits, compiles and closes timing** (0.2.1 build: blit row fast path, blit interrupt budget, late-flip pre-step, with `MUX_RESTRUCTURE ON` and register duplication off): Fitter 0 errors; 16,568 / 18,480 ALMs (90 %), 15,342 registers, block RAM 52 %. **Zero negative slack** at every corner on the 96 MHz core clock with the SDC's argued multicycles: setup +0.015 ns (slow 85 °C, on `sdram_ctrl` S_ARB -> SDRAM_A[11], the placement-sensitive path that has been within 0.1 ns on three builds now), +0.18 (slow 0 °C), +3.40 / +3.59 (fast); hold +0.286 / +0.277 (slow), +0.106 / +0.078 (fast 85 / 0 °C). The 0.2.0 build had 17,758 ALMs (96 %) and setup +0.16 / +0.40. Which slow corner is worst for setup flips between builds — always read both; a future build that misses on the S_ARB path should register SDRAM_A's address load one state earlier rather than argue a multicycle. |

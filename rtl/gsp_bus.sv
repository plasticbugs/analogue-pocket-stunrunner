...
// a write cycle, when the control_hi enable is set, copies 256 words (or
// 1024 in the expander region) from the latched row to the destination row
// through the SDRAM port.
//------------------------------------------------------------------------------
...
module gsp_bus (
    ...
    typedef enum logic [3:0] {IDLE, VRAM_WAIT, EXP0, EXP1, EXP2, EXP3, EXP_WAIT,
                              REG_RD, SRT_RD, SRT_RD_WAIT, SRT_WR, SRT_WR_WAIT, DONE} st_t;
    st_t st;
    logic  [1:0] exp_n;
    logic [15:0] exp_mask;            // data word driving the 4 masked writes
    logic [15:0] exp_color;
    logic [17:0] exp_base;
    logic [10:0] srt_cnt;             // words copied
    logic [10:0] srt_len;
    logic [17:0] srt_dst;
    logic [15:0] srt_word;
    ...
    always_ff @(posedge clk) begin
        ...
        pal_we_b  <= 1'b0;
        if (reset) begin
            st <= IDLE; sd_req <= 1'b0; sd_we <= 1'b0;
            ...
            case (st)
                ...
                // shift-register copy: read src[i], write dst[i]
                SRT_RD: begin
                    sd_addr <= VRAM_BASE + {6'd0, srt_src + {7'd0, srt_cnt}};
                    sd_we   <= 1'b0; sd_req <= 1'b1; st <= SRT_RD_WAIT;
                end
                SRT_RD_WAIT: if (sd_ack) begin
                    sd_req <= 1'b0; srt_word <= sd_rdata; st <= SRT_WR;
                end
                SRT_WR: begin
                    sd_addr <= VRAM_BASE + {6'd0, srt_dst + {7'd0, srt_cnt}};
                    sd_wdata <= srt_word; sd_be <= 2'b11;
                    sd_we   <= 1'b1; sd_req <= 1'b1; st <= SRT_WR_WAIT;
                end
                SRT_WR_WAIT: if (sd_ack) begin
                    sd_req <= 1'b0;
                    srt_cnt <= srt_cnt + 11'd1;
                    if (srt_cnt == srt_len - 11'd1) begin mem_ack <= 1'b1; vram_copied <= 1'b1; st <= IDLE; end
                    else st <= SRT_RD;
                end
                default: st <= IDLE;
            endcase

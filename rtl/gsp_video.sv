...
module gsp_video (
    ...
    // ---- pixel pipeline ------------------------------------------------------
    // x = pixel index within the line (2 per video clock)
    ...
    logic  [8:0] x_off;                   // pixel counter from heblnk
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            x_off <= '0; col_odd <= 1'b0; col_odd_next <= 1'b0; hcount_d <= '0; vs_line <= 1'b0;
        end else begin
            if (line_start) begin col_odd <= col_odd_next; col_odd_next <= col0[0]; vs_line <= vs_n; end
            if (cen_pix) begin
                hcount_d <= hcount;
                // x_off counts pixels from the left edge of the visible area
                if (in_h) x_off <= (hcount == r_heblnk && hcount != hcount_d) ? 9'd1 : x_off + 9'd1;
                else x_off <= '0;
            end
        end
    end
    ...
    wire  [9:0] xidx  = {1'b0, x_off} + {9'd0, col_odd};     // byte index into the 514-byte window
    ...
    always_ff @(posedge clk) begin
        if (cen_pix) begin
            ...
            rd_odd  <= xidx[0];
            show_d  <= show_sel;
            de_1 <= in_h && show_visible;
            hs_1 <= hs_n; vs_1 <= vs_line; hb_1 <= ~in_h; vb_1 <= ~show_visible;
            // stage 2: palette
